...
public class NoticeDAO {
	...
	public List<Notice> selectNoticeList(Connection conn) {
	      ...
	      try {
	         ...
	         while(rset.next()) {
	            ...
	            notice.setBoardContent(rset.getString("CONTENT"));
	...
	public List<Review> selectReviewList(Connection conn, int cPage, int numPerPage) {
		...
		try {
			...
			while(rset.next()) {
				...
				r.setMemberCode(rset.getString("member_code"));
				r.setBoardCate(rset.getString("board_cate"));
				r.setBoardTItle(rset.getString("board_title"));
				r.setBoardContent(rset.getString("board_content"));
				r.setBoardOrigin(rset.getString("board_origin"));
				r.setBoardRename(rset.getString("board_renamed"));
				r.setBoardDate(rset.getDate("board_date"));
				r.setBoardReadcnt(rset.getInt("board_readcnt"));
				r.setBoardLevel(rset.getInt("board_level"));
				r.setBoardRef(rset.getInt("board_ref"));
	...
	public int insertReview(Connection conn, Review r) {
		...
		try {
			pstmt = conn.prepareStatement(query);
			pstmt.setString(1,r.getMemberCode());
			pstmt.setString(2, r.getBoardTItle());
			pstmt.setString(3, r.getBoardContent());
			pstmt.setString(4, r.getBoardOrigin());
			pstmt.setString(5, r.getBoardRename());
			result = pstmt.executeUpdate();
		...
		System.out.println(result+"sgdfsdfgsdfg");
		return result;
	...
	public int updateReview(Connection conn, Review r) {
		...
		try {
			...
			pstmt.setString(3, r.getBoardOrigin());
			pstmt.setString(4,r.getBoardRename());
			pstmt.setInt(5, r.getBoardNo());
			result = pstmt.executeUpdate();
	...
	public List<Review> selectMostViewReview(Connection conn) {
		List<Review> rList = new ArrayList<Review>();
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		Review r = null;
		String query = prop.getProperty("selectMostViewReview");
	...
	public Review selectOneReview(Connection conn, int boardNo) {
		Review review = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		...
		try{
			...
			//쿼리문미완성
			pstmt.setInt(1, boardNo);
			System.out.println("reviewSelectOne@dao="+boardNo);
			//쿼리문실행
			//완성된 쿼리를 가지고 있는 pstmt실행(파라미터 없음)
			rset = pstmt.executeQuery();
			...
			if(rset.next()){
				...
				review.setMemberCode(rset.getString("member_code"));
